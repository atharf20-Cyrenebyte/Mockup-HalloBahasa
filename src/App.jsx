...
import { Book, Award, Home, MapPin, Users, Clock, LogOut, Filter, Calendar, ArrowLeft, CheckCircle, XCircle, Trash2 } from 'lucide-react';
...
function App() {
  ...
  const [form, setForm] = useState({ program: 'private', date: '', time: '', notes: '' });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-yellow-50 to-amber-100">
        ...
        <section className="container mx-auto px-4 py-20 text-center">
          ...
          <button onClick={() => setPage('login')} className="text-gray-800 px-8 py-4 rounded-lg text-lg hover:bg-yellow-500 transition font-semibold" style={{ backgroundColor: '#FFFC00' }}>Mulai Sekarang</button>
        </section>
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white rounded-xl shadow-md p-8">
              ...
              <form onSubmit={(e) => { e.preventDefault(); if (!form.date || !form.time) return alert('Lengkapi form!'); setBookings([...bookings, { id: bookings.length + 1, coachId: selCoach.id, coachName: selCoach.name, coachPhoto: selCoach.photo, customerName: "Demo", program: form.program, date: form.date, time: form.time, notes: form.notes, price: form.program === 'private' ? selCoach.price.private : selCoach.price.public, status: 'pending' }]); setForm({ program: 'private', date: '', time: '', notes: '' }); alert('🎉 Booking berhasil!'); setPage('my-bookings'); }} className="space-y-6">
                ...
                <div><label className="block font-semibold mb-2">Waktu</label><input type="time" value={form.time} onChange={(e) => setForm({...form, time: e.target.value})} className="w-full border-2 rounded-lg px-4 py-3" required /></div>
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          {mine.length === 0 ? <div className="bg-white rounded-xl shadow-md p-12 text-center"><p className="text-gray-600 mb-4">Belum ada booking</p><button onClick={() => setPage('coaches')} className="text-gray-800 px-6 py-2 rounded-lg font-semibold" style={{ backgroundColor: '#FFFC00' }}>Browse Coach</button></div> : <div className="space-y-4">{mine.map(b => <div key={b.id} className="bg-white rounded-xl shadow-md p-6"><div className="flex justify-between items-start"><div className="flex gap-4"><img src={b.coachPhoto} alt={b.coachName} className="w-16 h-16 rounded-full bg-gray-200" /><div><h3 className="text-xl font-bold mb-1">{b.coachName}</h3><p className="text-gray-600 mb-1">Program: {b.program === 'private' ? 'Private' : 'Public'}</p><p className="text-gray-600 mb-1">Tanggal: {b.date} | {b.time}</p><p className="font-bold text-amber-600">Rp {b.price.toLocaleString('id-ID')}</p>{b.notes && <p className="text-sm text-gray-500 mt-2">Catatan: {b.notes}</p>}</div></div><span className={`px-4 py-2 rounded-full text-sm font-semibold ${b.status === 'approved' ? 'bg-green-100 text-green-700' : b.status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>{b.status === 'approved' ? '✓ Approved' : b.status === 'rejected' ? '✗ Rejected' : '⏱ Pending'}</span></div></div>)}</div>}
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="grid md:grid-cols-4 gap-6 mb-8">
            <div className="bg-white rounded-xl shadow-md p-6"><h3 className="text-gray-600 mb-2">Coaches</h3><p className="text-4xl font-bold text-amber-600">{coaches.length}</p></div>
            <div className="bg-white rounded-xl shadow-md p-6"><h3 className="text-gray-600 mb-2">Bookings</h3><p className="text-4xl font-bold text-green-600">{bookings.length}</p></div>
            <div className="bg-white rounded-xl shadow-md p-6"><h3 className="text-gray-600 mb-2">Pending</h3><p className="text-4xl font-bold text-yellow-600">{bookings.filter(b => b.status === 'pending').length}</p></div>
            <div className="bg-white rounded-xl shadow-md p-6"><h3 className="text-gray-600 mb-2">Approved</h3><p className="text-4xl font-bold text-blue-600">{bookings.filter(b => b.status === 'approved').length}</p></div>
          </div>
          
          <h3 className="text-2xl font-bold mb-4">Manage Bookings</h3>
          {bookings.length === 0 ? <div className="bg-white rounded-xl shadow-md p-12 text-center"><p className="text-gray-600">Belum ada booking</p></div> : <div className="space-y-4">{bookings.map(b => <div key={b.id} className="bg-white rounded-xl shadow-md p-6"><div className="flex justify-between items-center"><div><h4 className="text-lg font-bold">{b.customerName} → {b.coachName}</h4><p className="text-gray-600">{b.program} | {b.date} {b.time}</p><p className="font-bold text-amber-600">Rp {b.price.toLocaleString('id-ID')}</p></div><div className="flex gap-2">{b.status === 'pending' && <><button onClick={() => setBookings(bookings.map(bk => bk.id === b.id ? {...bk, status: 'approved'} : bk))} className="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 flex items-center gap-2"><CheckCircle className="w-4 h-4" />Approve</button><button onClick={() => setBookings(bookings.map(bk => bk.id === b.id ? {...bk, status: 'rejected'} : bk))} className="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 flex items-center gap-2"><XCircle className="w-4 h-4" />Reject</button></>}{b.status !== 'pending' && <span className={`px-4 py-2 rounded-full text-sm font-semibold ${b.status === 'approved' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{b.status}</span>}</div></div></div>)}</div>}

          <h3 className="text-2xl font-bold mt-12 mb-4">Manage Coaches</h3>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">{coaches.map(c => <div key={c.id} className="bg-white rounded-xl shadow-md p-6"><img src={c.photo} alt={c.name} className="w-20 h-20 rounded-full mx-auto mb-4 bg-gray-200" /><h4 className="text-lg font-bold text-center mb-2">{c.name}</h4><p className="text-sm text-gray-600 text-center mb-4">{c.location}</p><button onClick={() => { if (confirm('Hapus coach?')) { setCoaches(coaches.filter(coach => coach.id !== c.id)); alert('Coach dihapus!'); } }} className="w-full bg-red-500 text-white py-2 rounded-lg hover:bg-red-600 flex items-center justify-center gap-2"><Trash2 className="w-4 h-4" />Hapus</button></div>)}</div>
        </div>
      </div>
    );
  }
